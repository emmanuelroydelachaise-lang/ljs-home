(() => {
  if (!window.supabase || window.__LJS_HOME_ROUTER__) return;
  window.__LJS_HOME_ROUTER__ = true;

  const originalCreateClient = window.supabase.createClient.bind(window.supabase);

  const mapDbName = (name) => {
    if (typeof name !== 'string') return name;
    return name.startsWith('ljs_') ? 'home_' + name.slice(4) : name;
  };

  const mapFunctionName = (name) => {
    if (name === 'manage-technician') return 'home-manage-technician';
    return name;
  };
